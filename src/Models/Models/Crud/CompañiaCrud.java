...
public class CompañiaCrud {
    private static final Map<String, Compañia> Bdcompañia= new HashMap<>();
    ...
    public List<String> ListaNombreCompañias()throws Exception{
        if(Bdcompañia.isEmpty()){
            throw new Exception("No existen compañias registradas");
        }
        List<String> NameCompañias = new ArrayList<>();
        for (Map.Entry<String, Compañia> entry : Bdcompañia.entrySet()) {
            String key = entry.getKey();
            NameCompañias.add(key);
        }
        return NameCompañias;
    }
