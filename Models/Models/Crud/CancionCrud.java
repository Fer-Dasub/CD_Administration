...
public class CancionCrud {
    private static Map<String, Cancion> BdCancions= new HashMap<>();
    public int agregarCancion(Cancion c) throws Exception{
        if(BdCancions.containsKey(c.getCancionNombre())){
            throw new Exception("Ya existe una Cancion con la Nombre "+c.getCancionNombre());
        }
        BdCancions.put(c.getCancionNombre(), c);
    ...
    public int actualizaCancion(Cancion c) throws Exception{
        if(!BdCancions.containsKey(c.getCancionNombre())){
            throw new Exception("La Nombre "+c.getCancionNombre()+" no esta asociada a ninguna Cancion");
        }
        BdCancions.put(c.getCancionNombre(), c);
            return BdCancions.size();
        }
    ...
    public List<Cancion> ListaDeCancions()throws Exception{
        ...
        for (Map.Entry<String, Cancion> entry : BdCancions.entrySet()){
            String CancionNombre = entry.getKey();
